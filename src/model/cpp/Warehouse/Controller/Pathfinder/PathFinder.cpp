...
std::array<Node *, 4> PathFinder::findNeighbours(Node &node, const IMoveMechanism &moveMechanism) const
{
    std::array<Node *, 4> neighbours{
        ...
        new Node(std::make_pair(node.coords.first, node.coords.second - 1),
                 DirectionVector<>::UP(),
                 (/*node.byTime + */ moveMechanism.getTimeCost(node.arriveOrientation, DirectionVector<>::UP())),
        ...
        new Node(std::make_pair(node.coords.first, node.coords.second + 1),
                 ...
                 (/*node.byTime + */ moveMechanism.getTimeCost(node.arriveOrientation, DirectionVector<>::DOWN())),
        ...
        new Node(std::make_pair(node.coords.first - 1, node.coords.second),
                 ...
                 (/*node.byTime + */ moveMechanism.getTimeCost(node.arriveOrientation, DirectionVector<>::LEFT())),
        ...
        new Node(std::make_pair(node.coords.first + 1, node.coords.second),
                 ...
                 (/*node.byTime + */ moveMechanism.getTimeCost(node.arriveOrientation, DirectionVector<>::RIGHT())),
                 (node.byEnergy + moveMechanism.getEnergyCost(node.arriveOrientation, DirectionVector<>::RIGHT())))

        // Waiting at current position
        // Node(node.coords,
        //      node.arriveOrientation,
        //      node.byTime + 1, node.byEnergy)
    };
                          ...
                          int startTime, const IMoveMechanism &moveMechanism) const
{
    ...
    std::unordered_set<const Node *> openLookup; // Todo saját hash_value nodera és tárolhatna Node-kat

    // TODO: Lehet Node-ra kéne saját hash_value és lehetne unordered_set<Node>
    ...
    while (!open.empty())
    {
        ...
        std::array<Node *, 4> neighbours = findNeighbours(*current, moveMechanism);
